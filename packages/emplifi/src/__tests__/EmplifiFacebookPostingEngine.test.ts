...
import { EmplifiFacebookPostingEngine } from "../EmplifiFacebookPostingEngine";
import { IContainer, IQueryEngine } from "@ts-flow/core";

jest.mock("axios");

describe("EmplifiFacebookPostingEngine", () => {
  const mockConfig = {
    apiKey: "test-api-key",
    ...
    outputProperty: "facebookResponse",
  };
  ...
  it("successfully creates a Facebook draft post", async () => {
    ...
    const engine: IQueryEngine = new EmplifiFacebookPostingEngine(
      "test-id",
      {} as IContainer,
      mockConfig,
    );
    ...
    await engine.execute(payload, (eventName, result) => {
      ...
      expect(result.facebookResponse).toEqual(mockPostResponse.data);
    });
    ...
    expect(axios.post).toHaveBeenLastCalledWith(
      "https://test.socialbakers.com/api/v1/profiles/12345/facebook/feed",
      {
        profile_id: "12345",
        message: "Test post test-value",
        link: expect.stringContaining("utm_campaign=test-campaign") as string,
        published: false,
      },
  ...
  it("processes template variables correctly", async () => {
    ...
    expect(axios.post).toHaveBeenLastCalledWith(
      expect.any(String),
      expect.objectContaining({
        message: "Test post custom-value",
        link: expect.stringContaining("custom-value"),
