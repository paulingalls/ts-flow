...
import { EmplifiTwitterPostingEngine } from "../EmplifiTwitterPostingEngine";
import { IContainer, IQueryEngine } from "@ts-flow/core";

jest.mock("axios");

describe("EmplifiTwitterPostingEngine", () => {
  const mockConfig = {
    apiKey: "test-api-key",
    ...
    outputProperty: "twitterResponse",
  };
  ...
  it("successfully creates a Twitter draft post", async () => {
    ...
    const engine: IQueryEngine = new EmplifiTwitterPostingEngine(
      "test-id",
      {} as IContainer,
      mockConfig,
    );
    ...
    await engine.execute(payload, (eventName, result) => {
      ...
      expect(result.twitterResponse).toEqual(mockPostResponse.data);
    });
    ...
    expect(axios.post).toHaveBeenLastCalledWith(
      "https://test.socialbakers.com/api/v1/profiles/12345/twitter/tweets",
      {
        profile_id: "12345",
  ...
  it("processes template variables correctly", async () => {
    ...
    expect(axios.post).toHaveBeenLastCalledWith(
      expect.any(String),
      expect.objectContaining({
        text: "Test post custom-value",
        link: expect.stringContaining("custom-value") as string,
