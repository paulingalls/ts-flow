...
@ContainerNode
export class EmplifiTwitterPostingEngine
  extends NodeBase
  implements IQueryEngine
{
  ...
  async execute(
    ...
    console.log("[EmplifiTwitterPostingEngine] Executing");
    ...
    if (!url.searchParams.has("utm_source")) {
      url.searchParams.append("utm_source", "twitter");
    }
    ...
    try {
      const response = await axios.post(
        `${this.baseUrl}api/v1/profiles/${this.profileId}/twitter/tweets`,
        postBody,
        {
          headers: {
      ...
      console.log(
        "[EmplifiTwitterPostingEngine] Post response:",
      ...
      console.error(
        "[EmplifiTwitterPostingEngine] Error creating Twitter post:",
        error,
      );
      throw error;
    }
