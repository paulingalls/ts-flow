...
export class BlueSkyLinkPostingEngine extends NodeBase implements IQueryEngine {
	...
	private async uploadImage(imageUrl: string, token: string): Promise<{ ref: { $link: string }; mimeType: string; size: number }> {
		// Download the image
	...
	private async getOgData(url: string): Promise<{ image?: string; description?: string; title?: string }> {
		try {
			...
			console.error("[BlueSkyLinkPostingEngine] Error fetching OG data:", error);
			return {};
		}
	}
	...
	async execute(
		...
		if (typeof processedText !== 'string') {
			throw new Error(`Failed to process text - invalid type returned: ${typeof processedText}`);
		}

		const processedUrl = keywordReplacement(this.postUrl, payload);
		if (typeof processedUrl !== 'string') {
			throw new Error(`Failed to process URL - invalid type returned: ${typeof processedUrl}`);
		}
		...
		if (ogData.image) {
			try {
				...
				console.error("[BlueSkyLinkPostingEngine] Error uploading image:", error);
			}
		}
		...
		try {
			...
			console.log("[BlueSkyLinkPostingEngine] Post response:", JSON.stringify(response.data));
