...
export function injectDataIntoJSONObject(data: JSONObject, template: JSONObject): JSONObject {
  const filledObject: JSONObject = structuredClone(template);
  return injectData(data, filledObject) as JSONObject;
}

function injectData(data: JSONObject, item: JSONObject | JSONValue): JSONObject | JSONValue {
