...
@ContainerNode
export class SlackYesNoQueryEngine
  extends NodeBase
  implements IQueryEngine, ISlackInteractiveListener
{
  private completeCallback:
    | ((completeEventName: string, result: JSONObject) => void)
    | null = null;
  ...
  constructor(id: string, container: IContainer, config: JSONObject) {
    ...
    const endpoint = this.container.createInstance(
      "SlackInteractiveEndpoint",
      "SlackInteractiveEndpoint",
      config,
  ...
  async execute(
    ...
    return axios
      ...
      .then((res) => {
        console.log(
          "SlackYesNoQueryEngine send query complete",
          res.status,
          res.data,
        );
      })
