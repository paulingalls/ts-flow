...
import {
  ISlackInteractiveListener,
  SlackInteractiveEndpoint,
} from "./SlackInteractiveEndpoint";
import axios, { AxiosHeaders } from "axios";
import {
  ISlackEventsListener,
  SlackEventsEndpoint,
} from "./SlackEventsEndpoint";

@ContainerNode
export class SlackHomePageTrigger
  extends NodeBase
  implements ITrigger, ISlackInteractiveListener, ISlackEventsListener
{
  ...
  constructor(id: string, container: IContainer, config: JSONObject) {
    ...
    const interactiveEndpoint = this.container.createInstance(
      "SlackInteractiveEndpoint",
      "SlackInteractiveEndpoint",
      config,
    ) as SlackInteractiveEndpoint;
    interactiveEndpoint.addListener(this);

    const eventsEndpoint = this.container.createInstance(
      "SlackEventsEndpoint",
      "SlackEventsEndpoint",
      config,
    ) as SlackEventsEndpoint;
  ...
  registerTriggerCallback(
    ...
    this.publishBlocksToHomePageForUser(
      process.env.DEFAULT_SLACK_USER_ID as string,
    );
  }
  ...
  onInteraction(payload: JSONObject): void {
    ...
    if (this.triggerBlockId === (actions[0]["block_id"] as string)) {
      const payload = injectDataIntoJSONObject(
        actions[0],
  ...
  private publishBlocksToHomePageForUser(userId: string) {
    ...
    axios
      ...
      .then((res) => {
        console.log(
          "SlackHomePageTrigger send query complete",
          res.status,
          res.data,
        );
      })
